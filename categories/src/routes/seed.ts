...
const DEFAULT_CATEGORIES = [
    ...
    { name: "Comedy", slug: "comedy", icon: "M12 18.75a6 6 0 006-6v-1.5m-6 7.5a6 6 0 01-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 01-3-3V4.5a3 3 0 116 0v8.25a3 3 0 01-3 3z" },
    { name: "Other", slug: "other", icon: "M9.568 3H5.25A2.25 2.25 0 003 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 005.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 009.568 3z" },
];

const router = express.Router();

router.post("/api/categories/seed", requireAuth, async (req: Request, res: Response) => {
    const existing = await Category.countDocuments();
    if (existing > 0) {
        return res.send({ message: `Already seeded (${existing} categories exist)`, count: existing });
    }

    const categories = [];
    for (const cat of DEFAULT_CATEGORIES) {
        const category = Category.build(cat);
        await category.save();
        categories.push(category);
    }

    res.status(201).send({ message: `Seeded ${categories.length} categories`, categories });
});

export { router as seedCategoryRouter };
